...
export const createMenuItem: RequestHandler = async (req, res) => {
  ...
  if (!restaurantId || !name || typeof price !== "number")
    return res
      .status(400)
      .json({ error: "restaurantId, name, price required" });
  ...
  const { data: full, error: getErr } = await supabaseAdmin
    .from("menu_items")
    .select("*")
    .eq("id", inserted.id)
    .single();
...
export const setMenuItemAvailability: RequestHandler = async (req, res) => {
  ...
  if (!itemId || typeof available !== "boolean")
    return res.status(400).json({ error: "itemId and available required" });
  const { data, error } = await supabaseAdmin
    .from("menu_items")
    .update({ available })
    .eq("id", itemId)
